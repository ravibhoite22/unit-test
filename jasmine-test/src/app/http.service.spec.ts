...
describe('HttpService', () => {
  ...
  it('should get users by id', () => {
    service.getUserById(1).subscribe((res: any) => {
      expect(res).toBeTruthy();
      expect(res.name).toBe('Leanne Graham');
    });
    testingController
      .expectOne('https://jsonplaceholder.typicode.com/users/1')
      .flush(mockList[0]);
  });
  ...
  it('should update post by id', () => {
    ...
    service.updatePost(1,data).subscribe((res: any) => {
      expect(res).toBeTruthy();
      expect(res.title).toBe('Testing One');
    });
    let response = {
      id: 1,
      title: 'Testing One',
